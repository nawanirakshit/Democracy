...
import `in`.democracy.app.kotlin.listeners.KotlinBaseListener
import `in`.democracy.app.kotlin.navigator.Navigator
import `in`.democracy.app.kotlin.viewmodel.KotlinBaseViewModel
import `in`.democracy.app.kotlin.viewmodel.common.ErrorType
import `in`.democracy.app.utils.extension.executeSafe
...
abstract class KotlinBaseActivity(@IdRes private val container: Int = android.R.id.content) :
    ...
    */
    ...
    override fun showLoading() {
        try {
            if (progress == null) {
                ...
                progressBar = deleteDialogView.findViewById(R.id.progress_bar)
                ...
                rotateAnimation.repeatMode = RotateAnimation.RESTART
                rotateAnimation.repeatCount = RotateAnimation.INFINITE
                rotateAnimation.interpolator = LinearInterpolator()
                progressBar?.animation = rotateAnimation
                progressBar?.animate()
    ...
    private fun listenForError() {
        baseViewModel?.errorObserver?.observe(this) {
            when (it.errorType) {
